...
4. 扫描所有 `[TODO]` 任务，选取第一个满足 `Depends On` 全部为 `[IMPLEMENTED]` 的任务（`none` 或 `N/A` 视为已满足）。若所有任务均未标记 `[TODO]`，提醒用户「当前无待拆解的任务，所有 task 已完成或已定义」并终止。若存在 `[TODO]` 任务但无任何任务的依赖已满足，醒示用户「所有待拆解 task 均有未满足的前置依赖，请先完成前置 task 后再运行本命令」并终止。从 tasks.md frontmatter 获取本轮 `change_type`。**每次执行仅处理一个任务，禁止跨 task 拆解。**
5. 将该任务拆解为实现规格。按以下顺序逐 spec 定义：
   ...
   d. **轻量声明**：`change_type` 为 chore / docs 的规格默认声明轻量（`- **轻量**: 是 — <理由>`）；其他类型若提案声明轻量，必须写明理由并经用户显式确认。
    e. 规格操作类型（新增/修改/废弃）由本轮 `change_type` 映射得出（tasks.md frontmatter；映射见下表）。
6. 检查是否存在重复或冲突定义
7. 生成差异提案并输出预览（含全部 spec 的状态变更）
8. 确认后写入 specifications.md（各 spec 的 `Task` 字段指向该 task），并将该 task 标题标记更新为 `[DEFINED]`
...
- **Depended By**: 废弃规格附加字段（通用模板不含），必填，列出引用本规格目标文件的全部模块，作为 code-generate 解引用的依据。以 `grep` 扫描代码库实际引用方为准（此刻代码仍为废弃前状态，引用方完整）；`architecture.md` 的 `# Dependency Index` 与历史归档作起点参考，不一致时以代码实测为准
...
- **Task**: Task N — 所属任务编号（spec→task 单向指针，code-review 据此分组递归上溯）
...
specification-define 是唯一负责将本轮 `change_type`（tasks.md frontmatter）映射为规格标题操作类型的入口。映射关系如下：
...
| 文件 | 用途 | 关键内容 |
|------|------|---------|
| `planning/tasks.md` | 任务列表 | 按 `## Task N` 标题顺序扫描，选取第一个标题标记 `[TODO]` 且依赖满足的任务，从详情字段提取 `Depends On`、`Description`，从 frontmatter 提取本轮 `change_type`。不跨 task 处理。 |
...
每次执行先受理 `[规格缺陷]` 规格（如有）；否则选取第一个满足依赖的 `[TODO]` 任务。再次执行时自动定位下一个满足依赖的 `[TODO]` 任务。无满足依赖的任务时醒示用户先完成前置 task。多个 task 并发可执行（如均无依赖）时按文档顺序依次推进；下游命令以 tasks.md 中首个 `[DEFINED]` task 为活跃 task，后续 task 的规格待该 task 达到 `[IMPLEMENTED]` 后进入 TDD 循环。

### 上游文档解析提示

- `planning/tasks.md` 的 frontmatter 含 `round`、`change_type`；各任务以 `## Task N: <name> [STATUS]` 为标题，任务详情以 `- **字段名**: 值` 格式书写
- `Depends On`、`Description` 为每个任务节的必填字段

## 输出

命令执行后：写入/修订 `planning/specifications.md`，更新 tasks.md 的任务标记；必要时按 AGENTS.md §通用回滚协议 输出回滚指令并重置状态。
...
- 规格标题操作类型必须与 tasks.md frontmatter 的 `change_type` 按映射表一致（chore / docs 按规格实际性质）
- 不允许重复定义相同规格
- 每个常规规格（非轻量）必须：
...
- 不包含任何实现代码
- 必须能映射回任务
- 每个 spec 的 `Task` 字段必须指向对应 task（spec→task 单向指针；tasks.md 不再持有规格列表）
- 废弃规格（removal）附加约束：
