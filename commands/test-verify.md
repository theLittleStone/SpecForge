---
name: test-verify
description: 独立测试运行与验证器——执行测试并根据调用上下文判断 PASS/FAIL，更新规格状态
---

## 功能说明

该命令是 TDD 流程中的独立验证节点，被 test-generate、code-generate、code-review 多阶段复用。
它不生成任何代码，只执行测试并基于调用上下文判断结果。

## 行为规则

执行流程：

1. 读取 `planning/specifications.md`，定位当前活跃 task（`planning/tasks.md` 中首个 `[DEFINED]` 的 task）
2. 识别需要验证的规格——根据调用上下文和规格当前状态确定：

   | 调用上下文 | 目标规格状态 | 期望结果 | 通过后新状态 | 失败后新状态 |
   |-----------|-------------|---------|------------|------------|
   | test-generate（RED 确认） | `[新增: 已定义]`、`[修改: 已定义]`、`[废弃: 待删除]` | **FAIL** | —（仅确认 RED，不更新状态） | —（醒示 agent） |
   | code-generate（GREEN 验证 — 新增/修改） | `[新增: 已实现]`、`[修改: 已实现]` | **PASS** | `[已测试]` | `[待修复]` |
   | code-generate（GREEN 验证 — 废弃） | `[废弃: 已删除]` | **PASS** | —（保持终态） | `[废弃: 待修复]` |
   | code-review（回归验证） | `[新增: 已测试]`、`[修改: 已测试]`、`[废弃: 已删除]` | **PASS** | —（审查结果由 code-review 裁决） | —（醒示 code-review） |

3. 检测测试框架（同 test-generate 逻辑）
4. 执行目标 spec 的测试（从 `Affected Files` 字段获取测试文件路径）
5. 分析测试结果：
   - 统计通过/失败/跳过的用例数
   - 提取失败用例的具体断言信息（预期值 vs 实际值、错误堆栈）
6. 根据上表映射更新规格状态，失败时将详情写入 `Issues` 字段，并递增 `Retry Count`

### 失败处理细则

#### 当 code-generate（GREEN 验证）FAIL 时：

1. 将失败信息写入 spec 的 `Issues` 字段：
   - 错误类型
   - 预期值 vs 实际值
   - 失败用例名称
2. 递增 `Retry Count`
3. 状态更新为 `[新增: 待修复]` 或 `[修改: 待修复]`
4. 醒示用户「测试未通过，请运行 `/code-generate` 根据 Issues 修复代码」
5. Retry Count ≥ 3 时额外醒示：「该规格已连续失败 3 次，建议检查测试用例是否正确或重新评估实现方案」

#### 当 code-review（回归验证）FAIL 时：

1. 不自动更新 spec 状态
2. 醒示用户「回归测试失败——重构可能引入了意外行为变更，请检查最近修改」

### 废弃规格的特殊处理

对于 `[废弃: 已删除]` 状态的规格：
- 调用来源为 code-generate（GREEN 验证）：
  - PASS → 保持 `[废弃: 已删除]` 终态，确认删除安全
  - FAIL → 状态更新为 `[废弃: 待修复]`，写入 Issues（副作用详情 + 受影响模块），递增 Retry Count。
    醒示用户「删除存在副作用，请运行 `/code-generate` 根据 Issues 修复受影响模块」
- 调用来源为 code-review（回归验证）：
  - PASS → 保持 `[废弃: 已删除]` 终态
  - FAIL → 醒示用户「回归测试失败——删除可能存在意外副作用，请检查最近修改」

## 输入来源

| 文件 | 用途 | 关键内容 |
|------|------|---------|
| `planning/specifications.md` | 实现规格 | 目标 spec 的 `Affected Files`（测试文件路径）、`Test Cases`、`Issues` |
| `planning/tasks.md` | 任务列表 | 定位当前活跃 task（首个 `[DEFINED]`） |

### 处理顺序

按 `planning/specifications.md` 中目标 spec 从上到下顺序依次验证。每次仅处理当前活跃 task 下的规格。

## 输出

执行测试并更新规格状态。输出格式：

```md
## 测试执行结果

| 规格 | 测试文件 | 通过 | 失败 | 跳过 | 结果 |
|------|---------|------|------|------|------|
| spec_a | src/__tests__/a.test.ts | 3 | 0 | 0 | ✓ PASS |
| spec_b | src/__tests__/b.test.ts | 1 | 2 | 0 | ✗ FAIL |

## 失败详情

### [新增: 待修复] spec_b
- **Issues**:
  - 测试「密码错误返回 401」失败：预期 401，实际 500
  - 测试「账户锁定返回 423」失败：预期 423，实际 200
- **Retry Count**: 2

## 下一步

- PASS → 提醒用户继续当前流程的下一步
- FAIL → 提醒用户运行 `/code-generate` 修复后重新运行本命令
```

## 约束

- 不生成任何代码或测试文件
- 仅执行验证，不修改测试代码
- 不允许跳过失败分析直接标记状态
- 废弃规格验证发现副作用时必须详细列出受影响模块
- 仅执行 `[自动化]` 测试（Test Double 对 test-verify 透明），不涉及 `[人工]` checklist
